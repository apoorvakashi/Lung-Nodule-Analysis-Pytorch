...
from combining_data.segm_data import Ct, Luna2dSegmentationDataset, TrainingLuna2dSegmentationDataset
...
def findPositiveSamples(start_ndx=0, limit=100):
    ...
    positiveSample_list = []
    for sample_tup in ds.candidates_list:
...
def showCandidate(series_uid, batch_ndx=None, **kwargs):
    ds = Luna2dSegmentationDataset(series_uid=series_uid, **kwargs)
    pos_list = [i for i, x in enumerate(ds.candidates_list) if x.isNodule_bool]
    ...
    plt.imshow(ct.hunits_arr[int(center_irc.index)], clim=clim, cmap='gray')

    subplot = fig.add_subplot(len(group_list) + 2, 3, 2)
    subplot.set_title('row {}'.format(int(center_irc.row)), fontsize=30)
    for label in (subplot.get_xticklabels() + subplot.get_yticklabels()):
        label.set_fontsize(20)
    plt.imshow(ct.hunits_arr[:,int(center_irc.row)], clim=clim, cmap='gray')
    ...
    for label in (subplot.get_xticklabels() + subplot.get_yticklabels()):
        label.set_fontsize(20)
    plt.imshow(ct.hunits_arr[:,:,int(center_irc.col)], clim=clim, cmap='gray')
    plt.gca().invert_yaxis()
...
def build2dLungMask(series_uid, center_ndx):
    ...
    ct_g = torch.from_numpy(ct.hunits_arr[center_ndx].astype(np.float32)).unsqueeze(0).unsqueeze(0).to('cuda')
