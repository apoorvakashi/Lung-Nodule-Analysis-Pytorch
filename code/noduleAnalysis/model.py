...
class LunaModel(nn.Module):
    ...
    def __init__(self, inp_channels=1, conv_channels=8):
        super().__init__()

        self.tail_batchnorm = nn.BatchNorm3d(1)

        self.block1 = LunaBlock(inp_channels, conv_channels)
    ...
    def forward(self, input_batch):
        ...
        batch_norm_out = self.tail_batchnorm(input_batch)

        block1_out = self.block1(batch_norm_out)
        block2_out = self.block2(block1_out)
        block3_out = self.block3(block2_out)
        block4_out = self.block4(block3_out)

        conv_flat = block4_out.view(block4_out.size(0), -1)

        linear_ouput = self.head_linear(conv_flat)

        act_output = self.head_softmax(linear_ouput)

        return linear_ouput, act_output
...
class LunaBlock(nn.Module):
    def __init__(self, inp_channels, conv_channels):
        ...
        self.conv1 = nn.Conv3d(
            inp_channels, conv_channels, kernel_size=3, padding=1, bias=True
        )
    ...
    def forward(self, input_batch):
        ...
        pool_out = self.maxpool(block_out)
